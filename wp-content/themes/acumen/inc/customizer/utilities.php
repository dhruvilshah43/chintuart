...
class Acumen_Customizer_Utilities {
	/**
	...
	public static function register_option( $option ) {
		...
		$defaults = apply_filters( 'acumen_customizer_defaults', array() );
		...
		if ( ! empty( $get_posts ) ) {
			foreach ( $get_posts as $get_post ) {
				...
				if ( empty( $title ) ) {
					...
					$title = sprintf( esc_html__( 'ID: %s No Title', 'acumen' ), $get_post->ID );
				}
	...
	static function section_visibility() {
		$options = array(
			'disabled'     => esc_html__( 'Disabled', 'acumen' ),
			'homepage'     => esc_html__( 'Homepage / Frontpage', 'acumen' ),
			'entire-site'  => esc_html__( 'Entire Site', 'acumen' ),
		);

		return apply_filters( 'acumen_section_visibility_options', $options );
	}
}
 ...
 * Initialize class
 */
$acumen_utilities = new Acumen_Customizer_Utilities();
