...
(function($) {
	...
	function initMainNavigation(container) {
		...
		var dropdownToggle = $('<button />', {
			...
			.append(acumenScreenReaderText.icon)
			.append($('<span />', {
				'class': 'screen-reader-text',
				text: acumenScreenReaderText.expand
			}));
		...
		container.find('.current-menu-ancestor > button')
			...
			.text(acumenScreenReaderText.collapse);
		...
		container.find('.dropdown-toggle').on( 'click', function(e) {
			...
			// jscs:enable
			screenReaderSpan.text(screenReaderSpan.text() === acumenScreenReaderText.expand ? acumenScreenReaderText.collapse : acumenScreenReaderText.expand);
		});
	}
	...
	(function() {
		...
		menuToggle.on('click.acumen', function() {
	...
	(function() {
		...
		function toggleFocusClassTouchScreen() {
			if (window.innerWidth >= 910) {
				$(document.body).on('touchstart.acumen', function(e) {
					if (!$(e.target).closest('.main-navigation li').length) {
						$('.main-navigation li').removeClass('focus');
					}
				});
				siteNavigation.find( '.menu-item-has-children > a, .page_item_has_children > a' ).on( 'touchstart.acumen', function( e ) {
				...
				siteNavigation.find('.menu-item-has-children > a').off('touchstart.acumen');
			}
		}

		if ('ontouchstart' in window) {
			$(window).on('resize.acumen', toggleFocusClassTouchScreen);
			toggleFocusClassTouchScreen();
		}

		siteNavigation.find('a').on('focus.acumen blur.acumen', function() {
	...
	$(document).ready(function() {
		$(window).on('load.acumen resize.acumen', function() {
			if (window.innerWidth < 910) {
