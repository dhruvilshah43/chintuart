...
/**
* Move parallax and video to hook
 * @since 2.0.5
 */
if( version_compare( OnePress_Plus::$theme_version, '2.0.5', '>=' ) ) {
    if ( $is_parallax ) {
        $hook_args = array(
            'image' => $images[0],
            'alpha' => '',
        ...
        $hook_args = array(
            'video_url' => $video_mp4,
            'video_webm_url' => $video_webm,
            'video_ogv_url' => $video_ogv,
            'image' => $images[0],
            '_bg_type' => 'video'
        );
    }
    ...
    do_action( 'onepress_before_section_part', 'hero', $hook_args );
      ...
      if ( $is_parallax ) {
        ...
        echo '<div class="parallax-bg no-img" style="background-image: url('.esc_url( $images[0]).');" data-stellar-ratio="0.2" data-stellar-offset-parent="true"></div>';
    } else {
        ...
        default:
            ...
            <div class="container"<?php echo $hero_content_style; ?>>
                <div class="hero__content hero-content-style<?php echo esc_attr( $layout ); ?>">
                    ...
                    <?php if ($hcl1_smalltext != '') echo '<div class="hero-small-text"> ' . apply_filters( 'onepress_the_content', wp_kses_post( $hcl1_smalltext ) ) . '</div>' ?>
